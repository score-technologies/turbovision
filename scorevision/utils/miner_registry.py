...
def _hf_revision_accessible(model_id: str, revision: Optional[str]) -> bool:
    ...
    try:
        ...
        logger.debug("[HF] model=%s revision=%s NOT accessible: %s", model_id, revision, e)
        return False
...
async def get_miners_from_registry(netuid: int) -> Dict[int, Miner]:
    ...
    logger.info("[Registry] extracting candidates (netuid=%s mechid=%s)", netuid, mechid)
        ...
        if m.chute_id:
            ...
            if not info:
                ...
                if slug_chutes and slug_chutes != (m.slug or ""):
                    ok = False
                    logger.info("[Registry] uid=%s: slug mismatch (chutes=%s, commit=%s)", uid, slug_chutes, m.slug)
                ch_rev = info.get("revision")
                if ch_rev and m.revision and str(ch_rev) != str(m.revision):
                    ok = False
                    logger.info("[Registry] uid=%s: revision mismatch (chutes=%s, commit=%s)", uid, ch_rev, m.revision)
        if ok:
            filtered[uid] = m
