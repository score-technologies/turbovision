...
# TODO: choose a single mode per miner deployment:
# "soccer_action", "cricket_delivery", or "snooker_ball_state".
...
async def handle_challenge(request: ChallengeRequest) -> ChallengeResponse:
    ...
    try:
        ...
        if MINER_MODE == "snooker_ball_state":
            prediction = predict_snooker_ball_state(request)
            processing_time = time.perf_counter() - start_time
            logger.info(
                "Snooker ball-state challenge completed: %s, time: %.1fs",
                request.challenge_id,
                processing_time,
            )
            return ChallengeResponse(
